...
import z from "zod";
import { IAddService, IEditSubService, ISubService } from "@/lib/schemas/services-schema";
...
const ServiceForm: React.FC<ServiceFormProps> = ({
  ...
  const initial = {
    ...
    enDesc:"",
    stopEnabled:false,
    serviceFile:{data:"",extension:""},
    subServices: [],
  };
  const [request, setRequest] = useState<z.infer<typeof IAddService>>(initial);
  const [isSubServiceFormOpen, setIsSubServiceFormOpen] = useState(false);
  const [editingSubService, setEditingSubService] = useState<z.infer<typeof ISubService> | z.infer<typeof IEditSubService> | undefined>(undefined);
  ...
  const handleSubmit = async () => {
    ...
    if (!validate.isValid) {
      if(validate?.errorsList?.subServices){
        toast.error(t(validate?.errorsList?.subServices?.[0]));
        return;
      }
